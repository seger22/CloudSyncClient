...
rs_mdfour64(rs_mdfour_t * m, const void *p)
{
    ...
    u_int32_t        A, B, C, D;
    const u_int32_t *X = (const u_int32_t *) p;
 ...
 * instead. */
inline static void
copy4( /* @out@ */ unsigned char *out, u_int32_t const x)
{
    out[0] = x;
    out[1] = x >> 8;
    out[2] = x >> 16;
...
inline static void
copy64( /* @out@ */ u_int32_t * M, unsigned char const *in)
{
    int i=16;
...
rs_mdfour_block(rs_mdfour_t *md, void const *p)
{
    ...
    if (ptrval & 3) {
        ...
        memcpy(M, p, 16 * sizeof(u_int32_t));
        rs_mdfour64(md, M);
    } else {
        rs_mdfour64(md, (const u_int32_t *) p);
    }
}
